...
class qclSolver:
    ...
    def __init__(self, struct, interval = 2, step = 0.05, side = 5.,TE = 400.):
        ...
        self.Perm = np.array([(self.struct.layers[int(self.index(z))].material.params['eps0']) for z in self.grid])*self.eps0
        self.lattconst = np.array([(self.struct.layers[int(self.index(z))].material.params['lattconst']) for z in self.grid])
    ...
    def setPotential(self, U, Hart = 0.):
        ...
        self.potential = np.array(
            [(self.struct.layers[int(self.index(z))].material.params['Ec']) for z in self.grid]) - U*self.grid /10**7 + Hart
    ...
    def selectPsi(self, deloc_ext = 1): 
        ...
        for i in range(0,len(eigs)):
            mass_sub[i] = step*(self.meff*(1+(eigs[i]-self.potential)/self.meff/Ep)*(psi[:,i]**2)).sum()
    ...
    def solvePoisson(self):
        ...
        Perm = np.array([(self.struct.layers[int(self.index(z))].material.params['eps0']) for z in self.grid])*self.eps0
    ...
    def w_lo_ph(self, k_i, init, fin, pi_pts = 150, istep = 0.2):
        ...
        qs_s = el**2*(h/skip*self.dop.sum()*10**6/(self.struct.length/10**9))/eps0/perm0/k_bol/self.TE   # perm0 should be avareged
        ...
        if k_f_e > 0:
            ...
            for i in range(0,pi_pts):
                I_e += h**2/q_e[i]*th_step*(Psi_i*Psi_f*(Psi_i*Psi_f)[:,np.newaxis]*np.exp(-q_e[i]*np.abs(z-z[:,np.newaxis]))).sum()    
        I_a = 0
        if k_f_a > 0:
            q_a = np.sqrt(k_f_a + k_i - 2*np.sqrt(k_f_a)*np.sqrt(k_i)*np.cos(theta) + qs_s)
            for i in range(0,pi_pts):
                I_a += h**2/q_a[i]*th_step*(Psi_i*Psi_f*(Psi_i*Psi_f)[:,np.newaxis]*np.exp(-q_a[i]*np.abs(z-z[:,np.newaxis]))).sum()
        
        w_e = m_f*(el**2)*E_lo/4/np.pi/(planck**3)/eps0*(1/perm_inf-1/perm0)*(1/(np.exp(E_lo/k_bol/self.TL)-1)+1)*I_e
        w_a = m_f*(el**2)*E_lo/4/np.pi/(planck**3)/eps0*(1/perm_inf-1/perm0)*(1/(np.exp(E_lo/k_bol/self.TL)-1))*I_a 
    ...
    def w_dop(self, k_i, init, fin, pi_pts = 150, istep = 0.2):
        ...
        k_bol = self.k_bol
        qs_s = el**2*(h/skip*dop.sum()/(self.struct.length/10**9))/eps0/perm0/k_bol/self.TE   # perm0 should be avareged
        
        q = np.sqrt(k_f + k_i - 2*np.sqrt(k_f)*np.sqrt(k_i)*np.cos(theta) + qs_s)
        I = 0
        
        for i in range(0,pi_pts):
            I += h**2/skip/q[i]**2*th_step*(dop[dop>0]/(Perm[dop>0])**2*(np.sum((Psi_i*Psi_f)*np.exp(-q[i]*np.abs(z-self.grid[dop>0,np.newaxis]/10**9)),axis = 1))**2).sum()
    ...
    def w_m(self, i, f, E_pts = 50): 
        ...
        I1 = (np.array([w_1(k**2) for k in kGrid])*kGrid*np.exp(-planck**2*kGrid**2/2/m/k_bol/TE)).sum()
        I2 = (np.array([w_2(k**2) for k in kGrid])*kGrid*np.exp(-planck**2*kGrid**2/2/m/k_bol/TE)).sum()
        I3 = (np.array([w_3(k**2) for k in kGrid])*kGrid*np.exp(-planck**2*kGrid**2/2/m/k_bol/TE)).sum()
        I4 = (np.array([w_4(k**2) for k in kGrid])*kGrid*np.exp(-planck**2*kGrid**2/2/m/k_bol/TE)).sum()
        ...
        for i in range(0,N):
            for f in range(0,N):
                if i!=f :
                    ...
                    R_forw[i][f] = 2*self.findOmega(i,f)**2*tau_ort/(1+delta_if**2*tau_ort**2)*qclSolver.sigma_b(delta_if, Population[i], mass_sub[i],TE)
                    R_back[f][i] = 2*self.findOmega(i,f)**2*tau_ort/(1+delta_if**2*tau_ort**2)*qclSolver.sigma_b(-delta_if, Population[f], mass_sub[f],TE)
    ...
    def gain_if(self,i,f,omega):
        ...
        koeff = el**2*z_aw**2*delta_E**2/(eps_0*c*planck**2*omega*self.refr*self.struct.length/10**7)
        
        deltaN = D_ei/Beta*np.log((1+np.exp(Beta*mu_i))/(1+qclSolver.Chi(delta,TE)*np.exp(Beta*mu_f)))+D_ef/Beta*np.log((1+qclSolver.Chi(-delta,TE)*np.exp(Beta*mu_i))/(1+np.exp(Beta*mu_f)))
        deltaN_ci = D_ei/Beta*np.log(1+np.exp(Beta*mu_i))+D_ef/Beta*np.log((1+qclSolver.Chi(-delta,TE)*np.exp(Beta*mu_i)))
        deltaN_cf = -D_ei/Beta*np.log(1+qclSolver.Chi(delta,TE)*np.exp(Beta*mu_f))-D_ef/Beta*np.log(1+np.exp(Beta*mu_f))
